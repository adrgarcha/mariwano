...
export const run = async ({ interaction }: CommandProps) => {
  ...
  const cantidad = isNaN(interaction.options.getInteger("cantidad")!)
    ? 0
    : interaction.options.getInteger("cantidad")!;
  const targetUserId =
    interaction.options.get("usuario")?.value || interaction.member?.user.id;
      ...
      case "new":
        {
          if (!targetUser) {
            interaction.editReply(
              "No se ha encontrado al usuario al que quieres invertir el dinero."
            );
            return;
          }
          ...
          if (user) {
            try {
              ...
              interaction.editReply(
                `Has invertido ${cantidad} en <@${targetUserId}>`
              );
            } catch (error) {
              console.error(
                `Ha ocurrido un error en el commando 'invertir': ${error}`
              );
            }
          } else {
            user = new User({
            ...
            if (user.investBankFactor == 0 && cantidad === 0) {
              ...
              if (userBalance - user.investBankFactor > 0) {
                ...
                let ganancias = Math.round(
                  -user.invested -
                    (userBalance - user.investBankFactor) *
                      factorial(Math.abs(user.investFactor))
                );
                interaction.editReply(
                  `Has perdido ${
...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand) =>
    subcommand.setName("info").setDescription(`Información sobre el comando`)
  )
  .addSubcommand((subcommand) =>
    subcommand
      .setName("claim")
  ...
  .addSubcommand((subcommand) =>
    subcommand
      ...
      .addIntegerOption((option) =>
        option
          .setName("cantidad")
          .setDescription(`La cantidad debe ser mayor a 10 000 gramos`)
          .setRequired(true)
      )
      .addMentionableOption((option) =>
        option
          .setName("usuario")
          .setDescription("Usuario al que quieres realizar la inversion")
          .setRequired(true)
      )
  );
