...
const getRandomQuestion = (
  ...
  const filteredQuestions = hardcore
    ? questions.filter((q) => q.dificultad > 1)
    : questions;
    ...
    collector.on("collect", (response) => {
      ...
      if (
        ...
        interaction.followUp(
          `¡Respuesta correcta! | ${amountWon} gramos de cocaína fueron agregadas a tu inventario.`
        );
      } else {
        interaction.followUp(
...
export const data = new SlashCommandBuilder()
  ...
  .addBooleanOption((option) =>
    option
      .setName("hardcore")
      .setDescription("Preguntas más difíciles pero ganas más gramos")
  );
