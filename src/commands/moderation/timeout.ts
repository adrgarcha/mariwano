...
module.exports = {
  ...
  run: async ({ interaction }) => {
    ...
    if (msDuration < 5000 || msDuration > 2.419e9) {
      ...
      await interaction.editReply(
        `La duracion del timeout no puede ser menor a 5 segundos o mayor a 28 dias.`
      );
      return;
    }
    ...
    if (targetUserRolePosition >= requestUserRolePosition) {
      await interaction.editReply(
        `No puedes hacer timeout al usuario ${targetUser} porque tiene el mismo o superior rol al tuyo.`
      );
      return;
    }
    ...
    if (targetUserRolePosition >= botRolePosition) {
      await interaction.editReply(
        `No puedo hacer timeout al usuario ${targetUser} porque tiene el mismo o superior rol que el mio.`
      );
      return;
    }
    ...
    try {
      ...
      if (targetUser.isCommunicationDisabled()) {
        ...
        await interaction.editReply(
          `El timeout del usuario ${targetUser} ha sido aumentado a ${prettysMs(
            msDuration,
            { verbose: true }
          )}.\nRazon: ${reason}.\n👏 Enhorabuena!`
        );
      ...
      await targetUser.timeout(msDuration, reason);
      await interaction.editReply(
        `El usuario ${targetUser} le han realizado un pedazo de timeout de ${prettysMs(
          msDuration,
          { verbose: true }
        )}.\nRazon: ${reason}.`
      );
